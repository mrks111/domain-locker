...
export default class SupabaseDatabaseService extends DatabaseService {
   ...
   */
        ...
        this.supabase.supabase
          ...
          .then(({ data, error }) => {
            ...
            const linkDomains = data.reduce(
              (acc: Record<string, { id: string; link_name: string; link_url: string; link_description?: string; domains: Set<string> }>, link: any) => {
            ...
            const linksWithDomains = Object.values(linkDomains).map(({ id, link_name, link_url, link_description, domains }) => ({
    ...
    updateLinkInDomains(linkId: string, linkData: {
      link_name?: string;
      link_url?: string;
      link_description?: string;
      domains?: string[];
      ...
      return this.listDomainNames()
        .pipe(
          ...
          switchMap(async (domainIds: string[]) => {
            const { data: existingLinks, error } = await this.supabase.supabase
              .from('domain_links')
              .select('domain_id')
              .eq('id', linkId);
